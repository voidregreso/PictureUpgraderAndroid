...
namespace wsdsb {
    ...
    void Encoder::PostProcess(const std::vector<Tensor_t> &input_tensor, std::vector<Tensor_t> &output_tensor, void *result) {
        CodeFormerResult_t* res = (CodeFormerResult_t*)result;
        ...
        res->output_tensors.resize(output_tensor.size());
        std::copy(output_tensor.begin(), output_tensor.end(), res->output_tensors.begin());
    }
